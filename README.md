...
Use `bin/deploy-2for2tires` for the direct deployment path. It builds the
...
Create `.kamal/secrets.2for2tires` and fill in the Rails master key, Managed
...
```sh
bin/deploy-2for2tires 15.222.63.120
```

Pass the Lightsail static IP directly to the deploy script:

```sh
bin/deploy-2for2tires YOUR_LIGHTSAIL_STATIC_IP
```

You can also use `LIGHTSAIL_STATIC_IP=your-lightsail-static-ip` as an
environment variable.

Run `bin/deploy-2for2tires` for each deploy. The app health endpoint is `/up`.
